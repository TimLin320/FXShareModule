...
// MARK: - UIButton扩展 - 点击
public extension AKMeta where Meta: UIButton {
    ...
    /// 类方法 初始化按钮 防止双击
    static func custom(_ title: String? = nil) -> Meta {
        let s = Meta(type: .custom)
        s.isExclusiveTouch = true
        s.doubleTap().title(title)
        return s
    }
}
